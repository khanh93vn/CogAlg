...
from .classes import Cgraph, CP, Cptuple, Cvec2d
...
def trace_edge(blob, mask__, verbose=False):
    ...
    while max_:  # queue of (y,x,P)s
        ...
        while maxQue:  # trace max_
            ...
            P = form_P(blob, CP(yx=Cvec2d(y,x), axis=Cvec2d(dy/g, dx/g), cells={(y,x)}, dert_=[(y,x,i,dy,dx,g,ma)]))
            edge.P_ += [P]
            if _P is not None:
...
def form_P(blob, P):
    ...
    P.ptuple = Cptuple(I, G, M, Ma, Cvec2d(Dy, Dx), L)
