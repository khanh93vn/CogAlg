...
def form_P(edge, yx, axis, root__):
    ...
    P = I, Dy, Dx, G, Ma, M, yx, axis, yx_, dert_, link_ = [i, gy, gx, g, ma, m, yx, axis, [yx], [pvdert], []]

    for dy, dx in [(-ay, -ax), (ay, ax)]: # scan in 2 opposite directions
        yx_.reverse(); dert_.reverse()
        ...
        while True:  # scan to blob boundary or angle miss
            ...
            # update P:
            m = ave_g - g
            I += i; Dy += dy; Dx += dx; G += g; Ma += ma; M += m
            yx_ = [(y, x)] + yx_; dert_ += [(i, gy, gx, g, ma, m)]
    ...
    yx = yx_[len(dert_) // 2]  # update center

    P[:] = I, Dy, Dx, G, M, Ma, yx, axis, yx_, dert_, link_  # repack/update P
