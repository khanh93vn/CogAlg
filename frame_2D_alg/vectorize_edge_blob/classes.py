...
class Cptuple(ClusterStructure):  # bottom-layer tuple of compared params in P, derH per par in derP, or PP
    ...
    M : int = 0
    Ma : float = 0.0
    angle : tuple = (0,0)  # in latuple only, replaced by float in vertuple
    aangle : tuple = (0,0,0,0)
...
class CderP(ClusterStructure):  # tuple of derivatives in P link: binary tree with latuple root and vertuple forks
    ...
    valT : tuple = (0,0)  # also of derH
    rdnT : tuple = (1,1)  # mrdn + uprdn if branch overlap?
    _P : object | None = None  # higher comparand
    P : object | None = None  # lower comparand
    roott : list = init_param([None, None])  # for der++
    box : tuple = (0,0,0,0)  # y0,yn, x0,xn: P.box+_P.box, or center+_center?
    L : int = 0
    fdx : object | None = None  # if comp_dx
'''
...
class CPP(CderP):
    ...
    link_t : list = init_param([[],[]])  # +ve rlink_, dlink_
    roott : tuple = (None, None)  # PPPm|PPPd containing this PP
    ...
    # fdiv : object | None = None  # if div_comp?
...
class Cgraph(ClusterStructure):  # params of single-fork node_ cluster per pplayers
    ...
    '''
    G : object | None = None  # same-scope lower-der|rng G.G.G., or [G0,G1] in derG, None in PP
    root : object | None = None  # root graph or derH G, element of ex.H[-1][fd]
    ...
    rng : int = 1
    box : tuple = (0,0,0,0,0,0)  # y,x, y0,yn, x0,xn
    nval : int = 0  # of open links: base alt rep
    alt_graph_ : list = init_param([])  # contour + overlapping contrast graphs
    alt_Graph : object | None = None  # conditional, summed and concatenated params of alt_graph_
