...
def image_to_blobs(image):
    ...
    # update adjacent blob
#    update_blob_adjacency(frame)
...
def form_stack_(P_, frame, y):  # Convert or merge every P into its stack of Ps, merge blobs
    ...
    while P_:
        ...
        if not up_fork_:
            ...
            new_stack = dict(I=I, G=G, Dy=0, Dx=Dx, S=L, Ly=1, y0=y, Py_=[P], blob=blob, down_fork_cnt=0, sign=s, pri_blob_= [])
                        ...
                        if not up_fork['blob'] is blob:
                            ...
                            for stack in stack_:
                                if not stack is up_fork:
                                    stack['blob'] = blob  # blobs in other up_forks are references to blob in the first up_fork.
        ...
        if next_stack_: # add prior stack's blob as prior blob
...
def form_blob(stack, frame):  # increment blob with terminated stack, check for blob termination and merger into frame
    ...
    I, G, Dy, Dx, S, Ly, y0, Py_, blob, down_fork_cnt, sign, pri_blob_ = stack.values()
    accum_Dert(blob['Dert'], I=I, G=G, Dy=Dy, Dx=Dx, S=S, Ly=Ly)
    ...
    if blob['open_stacks'] == 0:  # if number of incomplete stacks == 0, blob is terminated and packed in frame:
        ...
        if y0 == 0 or yn == frame['dert__'].shape[1] or x0 == 0 or xn == frame['dert__'].shape[2]:
            fopen = 1 # add open state (open = 1)
        else:
            fopen = 0 # add default internal state =0
        ...
        # sort list of stack_ by using y0 in ascending order
        blob['stack_'] = sorted(blob['stack_'], key=lambda k: k['y0'])
...
def update_blob_adjacency(frame):
    # categorize adjacent blobs into internal, external or open

    for ite_num,blob in enumerate(frame['blob__']):

        # 1st index = adjacent blobs
        ...
        for adj_blob in blob['adj_blobs']:
            ...
            if 'dert__' in adj_blob and adj_blob not in adj_blob_new_ and adj_blob is not blob:
                ...
                inp_y0 = blob['stack_'][0]['y0'] # first stack y
                inp_yn = blob['stack_'][-1]['y0'] + blob['stack_'][-1]['Ly'] # last stack y
                # inp_x0 = blob['stack_'][0]['Py_'][0]['x0'] # x0 in first P of first stack
                # inp_xn = blob['stack_'][-1]['Py_'][-1]['x0']+blob['stack_'][-1]['Py_'][-1]['L'] # x0 + L in last P of last stack

                pri_y0 = adj_blob['stack_'][0]['y0'] # first stack y
                pri_yn = adj_blob['stack_'][-1]['y0'] + adj_blob['stack_'][-1]['Ly'] # last stack y
                # pri_x0 = adj_blob['stack_'][0]['Py_'][0]['x0'] # x0 in first P of first stack
                # pri_xn = adj_blob['stack_'][-1]['Py_'][-1]['x0']+adj_blob['stack_'][-1]['Py_'][-1]['L'] # x0 + L in last P of last stack


                if adj_blob['fopen'] == 1:   # check for open
                    adj_blob_new_[0].append(adj_blob)
                    adj_blob_new_[1].append(2) # set open

                # not sure on "if not int_last_xn + 1 in pri_blob_dert__.mask"
                # check in y direction should be sufficient since they are in adjacent?
                elif inp_yn >= pri_yn and inp_y0 <= pri_y0:  # check for internal or external
                    adj_blob_new_[0].append(adj_blob)
                    adj_blob_new_[1].append(0) # set internal
                else:
                    adj_blob_new_[0].append(adj_blob)
