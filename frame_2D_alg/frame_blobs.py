...
def frame_blobs_root(image, intra=False, render=False, verbose=False, use_c=False):
    ...
    blob_, idmap, adj_pairs = flood_fill(der__t, sign__= ave-der__t[3] > 0,
                                         prior_forks='', verbose=verbose)  # der__t[3] is g, https://en.wikipedia.org/wiki/Flood_fill
    assign_adjacents(adj_pairs)  # forms adj_blobs per blob in adj_pairs
    I, Dy, Dx = 0, 0, 0
    for blob in blob_: I += blob.I; Dy += blob.Dy; Dx += blob.Dx
    frame = CBlob(I=I, Dy=Dy, Dx=Dx, root_der__t=der__t, der__t=der__t, rlayers=[blob_], box=(0, Y, 0, X))  # dlayers = []: no comp_a yet
    ...
    for y in range(height):
        for x in range(width):
            if idmap[y, x] == UNFILLED:  # ignore filled/clustered derts
                ...
                blob.der__t = tuple([param_der__t[y0:yn, x0:xn] for param_der__t in blob.root_der__t])  # add None__ for m__?
