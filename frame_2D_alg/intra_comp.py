...
from frame_blobs import Tidert, Tadert
...
def comp_r(dert__, rng, mask__=None):
    ...
    return Tidert(i__topleft, d_upleft__, d_upright__, g__, ri__), majority_mask__
...
def comp_a(dert__, mask__=None):  # cross-comp of gradient angle in 3x3 kernels
    ...
    ga__ = np.hypot(1-vday__, 1-vdax__)     # +1 for all positives
    # or ga__ = np.hypot(np.pi + np.arctan2(uday__, vday__), np.pi + np.arctan2(udax__, vdax__)?
    ...
    return Tadert(i__, g__, ga__, ri__, dy__, dx__, uday__, vday__, udax__, vdax__), majority_mask__
...
def comp_r_odd(dert__, ave, rng, root_fia, mask__=None):
    ...
    return Tidert(i__center, dy__, dx__, g__, m__), majority_mask__